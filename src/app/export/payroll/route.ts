...
export async function GET(request: Request) {
  ...
  const employees = allEmployees
    ...
    .filter((e) => (entity ? e.legalEntityId === entity : true))
    .filter((e) => isPaidInMonth(e, ym));

  const rows: unknown[][] = [[
    "Mã NV", "Họ tên", "Pháp nhân", "Lương cơ bản", "Phụ cấp", "Ngày nghỉ KL",
  ...
  for (const e of employees) {
    ...
    rows.push([
      e.code, e.fullName, entityName(e.legalEntityId), s.base, s.allowance, s.unpaidDays,
